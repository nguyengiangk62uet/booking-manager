...
import { hotelService, paymentInformationService } from '../../../services/index';
...
const HotelPayment = (props) => {
  ...
  const fetchPaymentInformationOnHotel = () => {
    hotelService.getAllPayment(hotel.id)
      .then(res => {
        setPayments(res.result.paymentInformations);
      })
  };
  const onFinish = (values) => {
    paymentInformationService.createPaymentInformation({...values, hotelId: hotel.id})
      .then(() => {
        notification.success({
          message: 'Thêm thanh toán thành công',
        });
        fetchPaymentInformationOnHotel();
      })
